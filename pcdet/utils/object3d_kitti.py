...
def get_2dobjects_from_label(label_file):
    with open(label_file, 'r') as f:
        lines = f.readlines()
...
#         self.box2d = np.array((float(label[1]), float(label[2]), float(label[3]), float(label[4]), float(label[5])), dtype=np.float32)

#         self.score = float(label[5])
...
class Object2d(object):
    def __init__(self, line):
        ...
        self.box2d = np.array((float(label[4]), float(label[5]), float(label[6]), float(label[7]), float(label[15])), dtype=np.float32)
